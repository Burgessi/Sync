...
<link rel="stylesheet"
	href="${root}/resources/vendors/bootstrap-icons/bootstrap-icons.css" />
		...
		<div class="container-fluid p-0">
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav ms-auto mb-lg-0">
					...
					<li class="nav-item dropdown me-3"><a class="nav-link"
						style="margin-right: -8px; margin-top: 0;"
						href="javascript:window.open('${root}/chat/main.do','채팅','width=1205px, height=745px, toolbar=no, menubar=no, left=100px, top=160px')">
							<i class="bi bi-chat-text bi-sub fs-4" style="color: #223055;"></i>
					</a></li>



					<li class="nav-item dropdown me-3"><a class="nav-link active"
						href="#" data-bs-toggle="dropdown" data-bs-display="static"
						aria-expanded="false"> <i
							class="bi bi-bell bi-sub fs-4"></i><span id="alarmCount"
							class="badge bg-danger"
							style="position: absolute; top: 7px; right: -1px; font-size: 10px;"></span>
					</a>


						<ul class="dropdown-menu dropdown-menu-end notification-dropdown"
							aria-labelledby="dropdownMenuButton" style="left: 13%; width:375px;">
			...
			<div class="dropdown">
				<a href="#" type="button" data-bs-toggle="dropdown"
					aria-expanded="false">
					<div class="user-menu d-flex">
						<div class="user-name text-end me-2"
							...
							<p class="mb-0 text-sm text-gray-600">${loginMember.deptName}
								${loginMember.posName}</p>
						</div>
								...
								<c:choose>
									<c:when test="${not empty infoDto.emp_profile_pic}">
										<img id="headerPreview"
											...
											src="${infoDto.emp_profile_pic}" alt="이미지" />
									</c:when>
									<c:otherwise>
										<img id="headerPreview"
											style="border-radius: 50%; width: 45px; height: 45px; object-fit: cover"
				...
				<ul class="dropdown-menu dropdown-menu-lg-end"
					aria-labelledby="dropdownMenuButton" style="min-width: 10.5rem;"
					data-ps-popper="static">
					<li>
						<h6 class="dropdown-header" style="margin-left: -15px">${loginDto.emp_name }님
							환영합니다!</h6>
					</li>
					<li><a class="dropdown-item" href="${root}/mypage.do"><img
							src="${root}/resources/img/mypage.png"
							style="margin-bottom: 5px; margin-right: 10px;">마이페이지</a></li>
					<li><a class="dropdown-item" href="#"><img
							src="${root}/resources/img/document.png"
							style="margin-bottom: 5px; margin-right: 10px;">내 결재문서</a></li>
					<c:if test="${loginMember.opLevel eq 2}">
						<li><a class="dropdown-item" href="${root}/foradmin/member/1"><i
								class="icon-mid bi bi-wallet me-2"></i> 관리자 메뉴</a></li>
					</c:if>
					<li>
						<hr class="dropdown-divider" />
					</li>
					<li><a class="dropdown-item" href="${root}/"><img
							src="${root}/resources/img/logout.png"
							style="margin-bottom: 5px; margin-right: 10px;">로그아웃</a></li>
				</ul>
			</div>
		</div>
	</nav>
...
<script type="text/javascript">
										...
										.each(
												...
												function(index, alarm) {
													...
													if (alarm.alarm_type == 'C') {
														...
														'<li class="dropdown-item notification-item" onclick="goBoard(\''
																+ alarm.bd_seq
																+ '\', \''
																+ alarm.alarm_id
																+ '\')">'
																+ '<a class="d-flex align-items-center" href="#">'
																+ '<div class="notification-icon me-2" style="color: ' + iconColor + ';">'
																+ icon
																+ '</div>'
	...
	function goBoard(bd_seq, alarm_id) {
		isRead(alarm_id);
		window.location.href = 'http://localhost:8080/Sync/board/detailBoard.do?bd_seq='
				+ bd_seq;
	}
	...
	function showNotification(message) {
		const notification = $('<div></div>').text(message).css({
			'position' : 'fixed',
			'bottom' : '20px',
			'right' : '20px',
		...
		$.ajax({
			...
			success : function(count) {
				...
				if (count > 0) {
	                ...
	                $('#alarmCount').hide(); //알림 수가 0이면 배지 숨김
	            }
			},
			error : function() {
